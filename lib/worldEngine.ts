...
export function calculateWorldState(game: GameState): WorldState {
  ...
  const escalationMomentum = clamp(
    Math.round(m.escalationRisk * 0.45 + p.militaryPressure * 0.3 + p.informationPressure * 0.15 + p.timePressure * 0.1),
    0,
    100,
  );

  const diplomaticOpening = clamp(
    Math.round(m.peaceWindow * 0.55 + m.allyCredit * 0.2 + game.diplomaticChannels.length * 4 - m.escalationRisk * 0.18),
    0,
    100,
  );

  const allianceSolidity = clamp(
    Math.round(m.allyCredit * 0.65 + m.allySupportShips * 5 + game.allyEngagementStage * 4 - game.allies[0].parliamentConstraint * 0.15),
    0,
    100,
  );

  const internationalLegitimacy = clamp(
    Math.round(m.governmentTrust * 0.45 + allianceSolidity * 0.25 + m.peaceWindow * 0.15 - game.mediaGovGap * 0.15),
    0,
    100,
  );

  const narrativeControl = clamp(
    Math.round(m.governmentTrust - game.mediaGovGap * 0.9 - m.anxiety * 0.25 + game.metrics.bureaucraticGrip * 0.25),
    -100,
    100,
  );
